...
def no_apples_for_blair_sub
  ...
  FROM cats
  join cattoys ON cats.id = cattoys.cat_id 
  WHERE cats.name != 'Blair' and cattoys.toy_id in (
          SELECT 
            toys.id 
          FROM 
            toys
...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
  SELECT 
    different_price_toys.name, different_price_toys.price
  FROM 
    toys AS same_price_toys
  JOIN 
    toys AS different_price_toys 
    ON same_price_toys.price = different_price_toys.price
  WHERE
    same_price_toys.name = 'Shiny Mouse' AND different_price_toys.name != 'Shiny Mouse';
