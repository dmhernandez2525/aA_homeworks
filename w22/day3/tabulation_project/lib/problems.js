...
function stepper(nums, memo = {}) {
    ...
    // j === 3
        ...
        for (let k = 0; k < times + 1 ; k++) {
            const element = table[j + k];

            if (!element){
                let flag = true
                while (flag) {
                    // let element1 = nums[j + k];
                    let element1 = j
                    // the index of nums that we want to look at 
                    // let element2 = element1 + (j + k);
                    let element2 = element1 + k;
                    console.log(element2)
                    if (nums[element2] + element2 > nums.length || element2 > nums.length) { 
                        table[0] = false
                        flag = false
                    }
                    else if(nums[element2] > 0) {
                        element1 = nums[element2]  + j
                    }else{
                        //  add tab 
                        j = true
                        flag = false
...
// let a = stepper([3, 1, 0, 5, 10])
let a = stepper([2, 3, 1, 1, 0, 4, 7, 8])
console.log(a)
