...
def below_average_years
  ...
  Movie.select('yr', 'COUNT(title) as bad_movies').where('score < 5').group('yr').order('bad_movies DESC')
...
def alphabetized_actors
  ...
  Actor.order('name ASC').limit(10) #pass in mac_os
...
def pulp_fiction_actors
  ...
  #Actor.joins(:movies).select(:id,:name).where('title = \'Pulp Fiction\'')10067
end
